  ...
  function boot() {
    ...
    var RENDERERS = {
      ...
      hub: function (p) {
        ...
        var quote = p.quote
          ? '<blockquote class="hero__quote" data-item><p>' + esc(t(p.quote.text)) + "</p>" +
            (t(p.quote.by) ? '<cite>' + esc(t(p.quote.by)) + "</cite>" : "") + "</blockquote>"
          : "";
        return head(p) +
      ...
      article: function (p) {
        ...
        var body = (p.sections || []).map(function (s) {
          var blocks = (s.blocks || []).map(function (b) {
            ...
            if (b.type === "note") return '<aside class="callout"><span class="material-symbols-rounded callout__icon" aria-hidden="true">' + esc(b.icon || "info") + '</span><div>' + esc(t(b.text)) + "</div></aside>";
            if (b.type === "table") {
              var thead = (b.columns || []).map(function (c) { return "<th>" + esc(t(c)) + "</th>"; }).join("");
              var tbody = (b.rows || []).map(function (row) {
                return "<tr>" + row.map(function (cell) { return "<td>" + esc(t(cell)) + "</td>"; }).join("") + "</tr>";
              }).join("");
              return '<div class="table-wrap table-wrap--prose"><table class="data-table"><thead><tr>' + thead + "</tr></thead><tbody>" + tbody + "</tbody></table></div>";
            }
            if (b.type === "ul") {
      ...
      table: function (p) {
        var lead = (p.lead || []).map(function (b) {
          return '<p class="page-lead">' + esc(t(b)) + "</p>";
        }).join("");
        var tiles = (p.tiles || []).map(function (s) {
          return '<div class="stat-card stat-card--' + esc(s.tone || "plain") + '" data-item>' +
            '<b class="stat-value">' + esc(String(s.value)) + "</b>" +
            '<span class="stat-label">' + esc(t(s.label)) + "</span></div>";
        }).join("");
        return head(p) + lead +
          (tiles ? '<div class="stat-grid stat-grid--tight">' + tiles + "</div>" : "") +
          '<div class="toolbar">' +
            '<input id="search" class="search" type="search" autocomplete="off" ' +
              'placeholder="' + (L.state.lang === "en" ? "Search…" : "搜尋…") + '" ' +
              'aria-label="' + (L.state.lang === "en" ? "Search" : "搜尋") + '" />' +
      ...
      /* ---- arcade: quiz / flashcards / glossary / matching game ---- */
      arcade: function (p) {
        var en = L.state.lang === "en";
        var tabs = [
          { key: "quiz",  icon: "quiz",      label: en ? "Quiz" : "隨堂測驗" },
          { key: "cards", icon: "style",     label: en ? "Flashcards" : "字卡翻卡" },
          { key: "gloss", icon: "menu_book", label: en ? "Glossary" : "術語速查" },
          { key: "match", icon: "extension", label: en ? "Match Game" : "配對遊戲" }
        ];
        var seg = tabs.map(function (b, i) {
          return '<button class="seg__btn' + (i === 0 ? " seg__btn--active" : "") + '" type="button" data-view="' + b.key + '">' +
            '<span class="material-symbols-rounded" aria-hidden="true">' + b.icon + "</span>" + esc(b.label) + "</button>";
        }).join("");
        return head(p) +
          '<div class="seg seg--arcade" role="tablist">' + seg + "</div>" +
          '<div id="arcadeView" class="arcade"></div>';
      }
    };
    ...
    var WIRE = {
      ...
      gallery: function (p) {
        ...
        function paint() {
          ...
          grid.innerHTML = rows.map(function (item) {
            ...
            return '<article class="card" tabindex="0" role="button" data-item data-slug="' + esc(item.slug) + '" ' +
              ...
              (t(item.meta) ? '<span class="card__meta">' + esc(t(item.meta)) + "</span>" : "") +
        ...
        function openItem(slug) {
          ...
          var tags = (item.tags || []).map(function (g) { return '<span class="tag">' + esc(g) + "</span>"; }).join("");
          var details = (item.details || []).map(function (d) {
            return '<div class="detail-row"><dt>' + esc(t(d.label)) + "</dt><dd>" + esc(t(d.value)) + "</dd></div>";
          }).join("");
        ...
        function paint() {
          ...
          tbody.innerHTML = rows.map(function (row) {
            return "<tr data-item>" + cols.map(function (c) {
              ...
              if (c.type === "tag") {
                var raw = row[c.key];
                var toneKey = typeof raw === "object" ? (raw.en || "") : String(raw == null ? "" : raw);
                var tone = toneKey.toLowerCase().replace(/[^a-z]+/g, "-").replace(/^-|-$/g, "");
                return '<td><span class="badge badge--' + esc(tone) + '">' + esc(cellText(row, c)) + "</span></td>";
              }
      ...
      arcade: function (p) {
        var en = L.state.lang === "en";
        var view = document.getElementById("arcadeView");
        var btns = [].slice.call(pageEl.querySelectorAll(".seg__btn"));
        var timer = null;
        teardowns.push(function () { if (timer) clearTimeout(timer); });

        /* ---- quiz: one question at a time, session-only score ---- */
        function quiz() {
          var qs = p.quiz || [], i = 0, score = 0, answered = false;
          function paint() {
            if (i >= qs.length) {
              view.innerHTML = '<div class="arcade-panel arcade-panel--center" data-item>' +
                '<p class="arcade-final">' + (en ? "Final score: " : "最終得分：") + "<b>" + score + " / " + qs.length + "</b></p>" +
                '<button class="btn" type="button" id="qRestart">' + (en ? "Try again" : "再玩一次") + "</button></div>";
              document.getElementById("qRestart").addEventListener("click", function () { i = 0; score = 0; paint(); });
              return;
            }
            var q = qs[i];
            view.innerHTML = '<div class="arcade-panel" data-item>' +
              '<p class="arcade-progress">' + (en ? "Question " + (i + 1) + " of " + qs.length + " · Score " + score
                                                  : "第 " + (i + 1) + " / " + qs.length + " 題　·　得分 " + score) + "</p>" +
              '<h3 class="arcade-q">' + esc(t(q.q)) + "</h3>" +
              '<div class="quiz-opts">' + (q.options || []).map(function (o, k) {
                return '<button class="quiz-opt" type="button" data-k="' + k + '">' + esc(t(o)) + "</button>";
              }).join("") + "</div>" +
              '<div class="quiz-explain" id="quizExplain" hidden></div>' +
              '<button class="btn" type="button" id="quizNext" hidden>' + (en ? "Next" : "下一題") + "</button></div>";
            answered = false;
            [].forEach.call(view.querySelectorAll(".quiz-opt"), function (b) {
              b.addEventListener("click", function () {
                if (answered) return;
                answered = true;
                ...
                if (right) score++;
                [].forEach.call(view.querySelectorAll(".quiz-opt"), function (x, xi) {
                  x.disabled = true;
                  if (xi === q.answer) x.classList.add("quiz-opt--right");
                });
                if (!right) b.classList.add("quiz-opt--wrong");
                var ex = document.getElementById("quizExplain");
                ex.hidden = false;
                ex.textContent = (right ? "✅ " : "❌ ") + t(q.explain);
                document.getElementById("quizNext").hidden = false;
              });
            });
            document.getElementById("quizNext").addEventListener("click", function () { i++; paint(); });
          }
          paint();
        }
        ...
        function cards() {
          var cs = p.cards || [], i = 0, flipped = false;
          function paint() {
            var c = cs[i];
            view.innerHTML = '<div class="arcade-panel arcade-panel--center" data-item>' +
              '<p class="arcade-progress">' + (i + 1) + " / " + cs.length + "</p>" +
              '<button class="flashcard' + (flipped ? " flashcard--flipped" : "") + '" type="button" id="flash" ' +
                'aria-label="' + (en ? "Flip card" : "翻面") + '">' +
                '<span class="flashcard__face flashcard__face--front">' + esc(t(c.front)) + "</span>" +
                '<span class="flashcard__face flashcard__face--back">' + esc(t(c.back)) + "</span>" +
              "</button>" +
              '<p class="arcade-note">' + (en ? "Click the card to flip it" : "點擊卡片翻面") + "</p>" +
              '<div class="arcade-row">' +
                '<button class="btn btn--ghost" type="button" id="fPrev">' + (en ? "Previous" : "上一張") + "</button>" +
                '<button class="btn" type="button" id="fNext">' + (en ? "Next" : "下一張") + "</button>" +
              "</div></div>";
            document.getElementById("flash").addEventListener("click", function () {
              flipped = !flipped;
              this.classList.toggle("flashcard--flipped", flipped);
            });
            document.getElementById("fPrev").addEventListener("click", function () { i = (i - 1 + cs.length) % cs.length; flipped = false; paint(); });
            document.getElementById("fNext").addEventListener("click", function () { i = (i + 1) % cs.length; flipped = false; paint(); });
          }
          paint();
        }
        ...
        function gloss() {
          var gs = p.glossary || [];
          view.innerHTML =
            '<div class="toolbar"><input id="gSearch" class="search" type="search" autocomplete="off" ' +
              'placeholder="' + (en ? "Search terms…" : "搜尋術語…") + '" aria-label="' + (en ? "Search" : "搜尋") + '" /></div>' +
            '<div class="accordion">' + gs.map(function (g) {
              return '<details class="acc-item" data-item data-q="' + esc((t(g.term) + " " + t(g.def)).toLowerCase()) + '">' +
          ...
          s.addEventListener("input", function () {
            var q = this.value.trim().toLowerCase();
            items.forEach(function (it) { it.style.display = (!q || (it.dataset.q || "").indexOf(q) !== -1) ? "" : "none"; });
          });
        }

        /* ---- match: memory pairs (stratagem ↔ theory), session-only ---- */
        function match() {
          var pairs = p.pairs || [];
          var tiles = [];
          pairs.forEach(function (pr, id) {
            tiles.push({ id: id, text: t(pr.a) });
            tiles.push({ id: id, text: t(pr.b) });
          });
          ...
          view.innerHTML = '<div class="arcade-panel" data-item>' +
            '<p class="arcade-progress" id="mStatus"></p>' +
            '<div class="match-grid">' + tiles.map(function (tile, idx) {
              return '<button class="match-tile" type="button" data-idx="' + idx + '" data-id="' + tile.id + '" ' +
                'aria-label="' + (en ? "Tile " : "牌 ") + (idx + 1) + '">' +
                '<span class="match-tile__back" aria-hidden="true">？</span>' +
                '<span class="match-tile__front">' + esc(tile.text) + "</span></button>";
            }).join("") + "</div>" +
            '<div class="arcade-row"><button class="btn btn--ghost" type="button" id="mRestart">' +
              (en ? "Shuffle & restart" : "重新洗牌") + "</button></div></div>";
          var status = document.getElementById("mStatus");
          function say() {
            var msg = en
              ? "Pairs " + matched + " / " + pairs.length + " · Flips " + moves
              : "配對 " + matched + " / " + pairs.length + "　·　翻牌 " + moves + " 次";
            if (matched === pairs.length) msg += en ? " — you win! 🎉" : "　—　全部配對成功！🎉";
            status.textContent = msg;
          }
          say();
          [].forEach.call(view.querySelectorAll(".match-tile"), function (b) {
            b.addEventListener("click", function () {
              if (lock || b.classList.contains("match-tile--open") || b.classList.contains("match-tile--matched")) return;
              b.classList.add("match-tile--open");
              open.push(b);
              if (open.length < 2) return;
              moves++; lock = true;
              var t1 = open[0], t2 = open[1];
              if (t1.dataset.id === t2.dataset.id) {
                t1.classList.add("match-tile--matched"); t2.classList.add("match-tile--matched");
                matched++; open = []; lock = false; say();
              } else {
                say();
                timer = setTimeout(function () {
                  t1.classList.remove("match-tile--open"); t2.classList.remove("match-tile--open");
                  open = []; lock = false;
                }, 750);
              }
            });
          });
          document.getElementById("mRestart").addEventListener("click", match);
        }

        var views = { quiz: quiz, cards: cards, gloss: gloss, match: match };
        btns.forEach(function (b) {
          b.addEventListener("click", function () {
            btns.forEach(function (x) { x.classList.remove("seg__btn--active"); });
            b.classList.add("seg__btn--active");
            (views[b.dataset.view] || quiz)();
          });
        });
        quiz();
      }
